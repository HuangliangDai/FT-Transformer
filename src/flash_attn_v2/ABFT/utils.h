...
// #define MYLIB_VERSION "ABFT"

// inline const char* get_include_version() {
...
template <typename Kernel_traits, bool A_in_regs = false, bool B_in_regs = false, typename Tensor0, typename Tensor1, typename Tensor2,
...
inline __device__ void gemm(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsA, Tensor4 const &tCsB,
    ...
    //ABFT
    Tensor rowsum_c = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>,Int<8>>{});
    clear(rowsum_c);
    Tensor rowsum_b = make_tensor<half_t>(partition_shape_B(tiled_mma, Shape<Int<8>, Int<8>>{}));
    clear(rowsum_b);

    Tensor rowsum_c2 = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>,Int<8>>{});
    clear(rowsum_c2);
    Tensor rowsum_b2 = make_tensor<half_t>(partition_shape_B(tiled_mma, Shape<Int<8>, Int<8>>{}));
    clear(rowsum_b2);
...
#pragma unroll
    for (int i = 0; i < size<2>(tCrA); ++i) {
        ...
        // ABFT
        clear(rowsum_b);
        for(int dim_0 = 0; dim_0 < size<0>(tCrB); dim_0++){
            for(int dim_1 = 0; dim_1 < size<1>(tCrB); dim_1++){
                rowsum_b(dim_0, 0, 0) += tCrB(dim_0, dim_1, i);
            }
        }
        ...
        for(int dim_0 = 0; dim_0 < size<0>(tCrB); dim_0++){
            for(int dim_1 = 0; dim_1 < size<1>(tCrB); dim_1++){
                rowsum_b2(dim_0, 0, 0) += tCrB(dim_0, dim_1, i);
            }
        }
        ...
        //ABFT
        cute::gemm(tiled_mma, tCrA(_, _, i), rowsum_b(_, _, 0), rowsum_c);
        cute::gemm(tiled_mma, tCrA(_, _, i), rowsum_b2(_, _, 0), rowsum_c2);
    }
 
    // Verification
#pragma unroll
    for(int dim_1 = 1; dim_1 < size<1>(rowsum_c); dim_1++){
        rowsum_c(0, 0, 0) += rowsum_c(0, dim_1, 0);
    }
    

#pragma unroll
    for(int dim_0 = 1; dim_0 < size<0>(rowsum_c); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(rowsum_c); dim_1++){
            rowsum_c(0, 0, 0) += rowsum_c(dim_0, dim_1, 0);
        }
    }

    // // fuse softmax & dmr
    // rowsum_c(0,0,0) = exp2f(rowsum_c(0,0,0));

#pragma unroll
    for(int dim_0 = 0; dim_0 < size<0>(acc); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(acc); dim_1++){
            for(int dim_2 = 0; dim_2 < size<2>(acc); dim_2++){
                rowsum_c(0, 0, 0) -= acc(dim_0, dim_1, dim_2);
            }
        }
    }

    acc(0,0,0) = (rowsum_c(0,0,0) == 0) ? 1.0 : acc(0,0,0); // for test only; may produce incorrect attention results

    // Verification
#pragma unroll
    for(int dim_1 = 1; dim_1 < size<1>(rowsum_c2); dim_1++){
        rowsum_c2(0, 0, 0) += rowsum_c2(0, dim_1, 0);
    }
    

#pragma unroll
    for(int dim_0 = 1; dim_0 < size<0>(rowsum_c2); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(rowsum_c2); dim_1++){
            rowsum_c2(0, 0, 0) += rowsum_c2(dim_0, dim_1, 0);
        }
    }

    // // fuse softmax & dmr
    // rowsum_c2(0,0,0) = exp2f(rowsum_c2(0,0,0));

#pragma unroll
    for(int dim_0 = 0; dim_0 < size<0>(acc); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(acc); dim_1++){
            for(int dim_2 = 0; dim_2 < size<2>(acc); dim_2++){
                rowsum_c2(0, 0, 0) -= acc(dim_0, dim_1, dim_2);
            }
        }
    }

    acc(0,0,0) = (rowsum_c2(0,0,0) == 0) ? 1.0 : acc(0,0,0); // for test only; may produce incorrect attention results
}

////////////////////////////////////////////////////////////////////////////////////////////////////

template <typename Tensor0, typename Tensor1, typename Tensor2, typename Tensor3, typename TiledMma, typename TiledCopy,
          typename ThrCopy, typename Tensor4, typename Tensor5>
inline __device__ void gemm_A_in_regs(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const &tCsB,
                                      TiledMma tiled_mma, TiledCopy smem_tiled_copy_B, ThrCopy smem_thr_copy_B, Tensor4 check_c, Tensor5 check_c2) {
    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(acc));   // MMA_M
    CUTE_STATIC_ASSERT_V(size<1>(tCrB) == size<2>(acc));   // MMA_N
    CUTE_STATIC_ASSERT_V(size<2>(tCrA) == size<2>(tCrB));  // MMA_K
    Tensor tCrB_copy_view = smem_thr_copy_B.retile_D(tCrB);
...
#pragma unroll
    for (int i = 0; i < size<2>(tCrA); ++i) {
        ...
        cute::gemm(tiled_mma, tCrA(_, _, i), tCrB(_, _, i), acc);
        //ABFT_2
        cute::gemm(tiled_mma, tCrA(_, _, i), rowsum_b(_, _, 0), check_c);
         //ABFT_2
        cute::gemm(tiled_mma, tCrA(_, _, i), rowsum_b2(_, _, 0), check_c2);
    }
}
