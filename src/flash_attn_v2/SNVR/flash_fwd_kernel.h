...
inline __device__ void softmax_rescale_o(Tensor0 &scores, Tensor1 &scores_max, Tensor1 &scores_sum, Tensor2 &acc_o,
    ...
    if (Is_first) {
        ...
        // duplication instruction to simulate the overhead of selective restriction, which will generate incorrect attention results
        flash::scale_apply_exp2(scores, scores_max, softmax_scale_log2);
...
//     acc_o(0,0,0) = (checksum_o(0,0,0) > 1 || checksum_o(0,0,0) < -1) ? 1.0 : acc_o(0,0,0);
...
//     acc_o(0,0,0) = (checksum_o2(0,0,0) > 1 || checksum_o2(0,0,0) < -1) ? 1.0 : acc_o(0,0,0);
