...
#pragma unroll
    ...
    //ABFT_2
    Tensor checksum_o = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>, Int<8>>{});
    clear(checksum_o);

    Tensor checksum_o2 = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>, Int<8>>{});
    clear(checksum_o2);
...
#pragma unroll
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        flash::gemm<Kernel_traits, /*A_in_regs=*/Kernel_traits::Is_Q_in_regs>(acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma,
        ...
        flash::gemm_A_in_regs(acc_o, tOrP, tOrVt, tOsVt, tiled_mma, smem_tiled_copy_V, smem_thr_copy_V, checksum_o, checksum_o2);
        // if (cute::thread0()) { print(scores); }
    ...
    // ABFT_2 Verification
#pragma unroll
    for(int dim_1 = 1; dim_1 < size<1>(checksum_o); dim_1++){
        checksum_o(0, 0, 0) += checksum_o(0, dim_1, 0);
    }
        

#pragma unroll
    for(int dim_0 = 1; dim_0 < size<0>(checksum_o); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(checksum_o); dim_1++){
            checksum_o(0, 0, 0) += checksum_o(dim_0, dim_1, 0);
        }
    }

#pragma unroll
    for(int dim_0 = 0; dim_0 < size<0>(acc_o); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(acc_o); dim_1++){
            for(int dim_2 = 0; dim_2 < size<2>(acc_o); dim_2++){
                checksum_o(0, 0, 0) -= acc_o(dim_0, dim_1, dim_2);
            }
        }
    }

    acc_o(0,0,0) = (checksum_o(0,0,0) > 1 || checksum_o(0,0,0) < -1) ? 1.0 : acc_o(0,0,0); // for overhead analysis only, may cause incorret attention results; set the threshold according to the required precision



// ABFT_2 Verification
#pragma unroll
    for(int dim_1 = 1; dim_1 < size<1>(checksum_o2); dim_1++){
        checksum_o2(0, 0, 0) += checksum_o2(0, dim_1, 0);
    }
        

#pragma unroll
    for(int dim_0 = 1; dim_0 < size<0>(checksum_o2); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(checksum_o2); dim_1++){
            checksum_o2(0, 0, 0) += checksum_o2(dim_0, dim_1, 0);
        }
    }
...
#pragma unroll
    for(int dim_0 = 0; dim_0 < size<0>(acc_o); dim_0++){
        for(int dim_1 = 0; dim_1 < size<1>(acc_o); dim_1++){
            for(int dim_2 = 0; dim_2 < size<2>(acc_o); dim_2++){
                checksum_o2(0, 0, 0) -= acc_o(dim_0, dim_1, dim_2);
            }
        }
    }

    acc_o(0,0,0) = (checksum_o2(0,0,0) > 1 || checksum_o2(0,0,0) < -1) ? 1.0 : acc_o(0,0,0); // for overhead analysis only, may cause incorret attention results; set the threshold according to the required precision
