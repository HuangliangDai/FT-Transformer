...
#pragma unroll
    ...
    // fuse softmax & dmr
    rowsum_c(0,0,0) = exp2f(rowsum_c(0,0,0)); // for overhead analysis only
...
#pragma unroll
    ...
    acc(0,0,0) = (rowsum_c(0,0,0) == 0) ? 1.0 : acc(0,0,0); // Set the threshold according to the required precision
...
#pragma unroll
    ...
    // fuse softmax & dmr
    rowsum_c2(0,0,0) = exp2f(rowsum_c2(0,0,0)); // for overhead analysis only
...
#pragma unroll
    ...
    acc(0,0,0) = (rowsum_c2(0,0,0) == 0) ? 1.0 : acc(0,0,0); // Set the threshold according to the required precision
}
