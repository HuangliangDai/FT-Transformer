...
public:
    explicit Tester(size_t batch = 2, size_t seq_q = 256, size_t seq_k = 256, size_t head_q = 32, size_t head_k = 32,
        ...
        // std::cout << "m_cu_seq_q: " << m_cu_seq_q[1] << std::endl;
        ...
        // std::cout << "m_cu_seq_q_dev: " << (m_cu_seq_q_dev == nullptr) << std::endl;
...
private:
    ...
    void profile(Func &&flash_attention, const std::string &name) {
        ...
        FLOG("execution complete, profiling time: %.3f ms (%.2f%%), throughput: %.3f TFLOPS (%.2f%%)",
